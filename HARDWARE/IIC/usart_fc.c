...
void FutabaSBUS_update_channels(void) {
	...
	if( USART_GetITStatus(USART3,USART_IT_RXNE) )
	{
		...
		switch(state)
		{
		  ...
		  case 1:
			   ...
			   if((sbus_data_i[byteCNT]==0&&sbus_data_i[byteCNT-1]==0))
				{
				for(i=0;i<25;i++)
					sbus_data[i]=sbus_data_i[i];
				FutabaSBUS_update_channels();
				state=0;
				byteCNT=0;
...
//		
	  //FutabaSBUS_update_channels(com_data);
